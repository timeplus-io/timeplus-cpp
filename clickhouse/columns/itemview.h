...
private:
    ...
    inline auto ConvertToStorageValue(const T& t) {
        ...
        } else if constexpr (std::is_fundamental_v<T> || std::is_same_v<Int128, std::decay_t<T>> || std::is_same_v<UInt128, std::decay_t<T>> || std::is_same_v<Int256, std::decay_t<T>> || std::is_same_v<UInt256, std::decay_t<T>>) {

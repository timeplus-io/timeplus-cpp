...
int main(int argc, char* argv[]) {
    ...
    config.client_options.compression_method = CompressionMethod::LZ4;
    ...
    constexpr double execution_time_sec = 600;
    int block_id = 1;
    for (;;) {
        auto start = std::chrono::high_resolution_clock::now();
        while (true) {
            auto result = tp.Insert("insert_benchmark_test", block, std::to_string(block_id));
            if (result.ok()) {
                break;
            }
    ...
    std::cout << "EPS: " << block->GetRowCount() * block_id / total_time << std::endl;

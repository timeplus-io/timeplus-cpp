...
//              << std::endl;
    ...
    throw timeplus::OpenSSLError(prefix + std::to_string(error) + " : " + reason_str);
}

void configureSSL(const timeplus::SSLParams::ConfigurationType & configuration, SSL * ssl, SSL_CTX * context = nullptr) {
    ...
    for (const auto & kv : configuration) {
        ...
        if (err == 2 || (err == 1 && !value_present))
            ...
            throw timeplus::OpenSSLError("Failed to configure OpenSSL: command '" + kv.first + "' needs no value");
        else if (err == -2)
            throw timeplus::OpenSSLError("Failed to configure OpenSSL: unknown command '" + kv.first + "'");
        else if (err == -3)
            throw timeplus::OpenSSLError("Failed to configure OpenSSL: command '" + kv.first + "' requires a value");
        else
            throw timeplus::OpenSSLError("Failed to configure OpenSSL: command '" + kv.first + "' unknown error: " + std::to_string(err));
    }
}
...
SSL_CTX * prepareSSLContext(const timeplus::SSLParams & context_params) {
    ...
    if (!ctx)
        throw timeplus::OpenSSLError("Failed to initialize SSL context");
...
auto convertConfiguration(const decltype(timeplus::ClientOptions::SSLOptions::configuration) & configuration)
{
    auto result = decltype(timeplus::SSLParams::configuration){};
...
timeplus::SSLParams GetSSLParams(const timeplus::ClientOptions& opts) {
    const auto& ssl_options = *opts.ssl_options;
    return timeplus::SSLParams{
    ...
    , ssl_(SSL_new(context.getContext()), &SSL_free)
{
    auto ssl = ssl_.get();
    if (!ssl)
        throw timeplus::OpenSSLError("Failed to create SSL instance");
    ...
    if (const auto verify_result = SSL_get_verify_result(ssl); !ssl_params.skip_verification && verify_result != X509_V_OK) {
        ...
        throw timeplus::OpenSSLError("Failed to verify SSL connection, X509_v error: "
